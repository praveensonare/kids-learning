...
import { ArrowLeft, BookOpen, Sparkles, FileText, Lightbulb, Target } from 'lucide-react';
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs md:text-sm text-gray-700 font-semibold">
                {classData.name} · {subjectData.name} · Chapter {chapter?.number}
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-4 py-8 md:py-12 relative z-10">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-gray-700 font-semibold">
            Select a topic to dive deep into learning with theory, examples, and practice problems!
          </p>
        </motion.div>
        ...
        {!loading && chapter && chapter.topics.length > 0 && (
          <div className="max-w-4xl mx-auto">
            {/* Chapter Info Card */}
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-purple-200 p-4 md:p-6 mb-6"
            >
              <div className="flex items-start gap-3">
                <BookOpen className="w-6 h-6 text-purple-600 flex-shrink-0 mt-1" />
                <div>
                  <h3 className="text-xl md:text-2xl font-black bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">
                    Chapter {chapter.number}: {chapter.title}
                  </h3>
                  <p className="text-sm text-gray-600 mb-3">
                    {chapter.topics.length} {chapter.topics.length === 1 ? 'topic' : 'topics'} · Click any topic to begin
                  </p>
                  {/* Keywords */}
                  <div className="flex flex-wrap gap-2">
                    {chapter.keywords.map((keyword, idx) => (
                      <span
                        key={idx}
                        className="bg-purple-50 text-purple-700 px-2 py-1 rounded-md text-xs font-semibold border border-purple-200"
                      >
                        {keyword}
                      </span>
                    ))}
            ...
            {/* Topics List */}
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border-2 border-purple-200 divide-y divide-gray-200">
              {chapter.topics.map((topic, index) => (
                ...
                >
                  <Link href={`/${classId}/${subjectId}/${chapterId}/${topic.id}`}>
                    <motion.div
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {(topic.problemsEasy || topic.problemsMedium || topic.problemsDifficult) && (
                              <span className="bg-pink-50 text-pink-700 px-2 py-0.5 rounded-md text-xs font-semibold border border-pink-200 flex items-center gap-1">
            ...
            >
              <div className="flex items-center gap-3 text-sm text-gray-700">
                ...
                <p className="font-semibold">
                  Each topic includes comprehensive theory, worked examples, interactive worksheets, and practice problems at three difficulty levels
                </p>
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className="bg-white/95 rounded-3xl shadow-xl p-8 md:p-12 max-w-md mx-auto border-2 border-purple-200">
              ...
              <p className="text-gray-600 text-base md:text-lg">
                Topics for this chapter are being prepared. Check back soon!
              </p>
            </div>
          </motion.div>
        )}
